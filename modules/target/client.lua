...
function B1.getTarget() return target end
...
-- Target functions
function B1.addZone(name, coords, length, width, options, targetoptions)
...
function B1.addEntity(entities, options, isNet, noconvert)
    if target == 'qb-target' then
...
function B1.addModel(models, options)
    if target == 'qb-target' then
...
function B1.removeEntity(entities, options)
    if target == 'qb-target' then
...
function B1.removeModel(models, options)
    if target == 'qb-target' then
...
function B1.addTargetBone(bones, options)
    if target == 'qb-target' then
        return export:AddTargetBone(bones, options)
    elseif target == 'ox_target' then
        B1.log('error', 'B1.addTargetBone does not support ox_target at the moment. We are sorry for this inconvenient.')
        return
    end
end
